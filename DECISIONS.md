...
## ADR-028: Implemented the Home page from an approved Claude Design project instead of another freehand attempt

**Date:** 2026-08-07

**Context:** Three rounds of freehand design work (ADR-023 through ADR-025) all drew "unprofessional"/"so bad" feedback, cumulating in ADR-026/027's full reset to a blank page. The user then supplied an actual design artifact to implement against instead: a Claude Design project (id `516f7668-7d03-4ad3-a7ab-e7df47d62afa`) containing a full site design — `Unda Solutions Website - Home v1.0.dc.html` plus About/Careers/Contact/Ecosystem/Insights/Work, a design system doc, and a logo/identity strategy doc — accessed via the `DesignSync` MCP tool. Scope for this round was explicitly limited to the Home file. Reading it (plus its `image-slot.js` and `support.js` imports) surfaced the specific fix for the recurring placeholder-image complaint (ADR-024/025): the design's own `<image-slot>` custom element renders, when empty, a neutral dashed-border frame with a small icon and a per-instance caption describing what photo belongs there — never a repeated brand mark. That empty-state pattern, not the element's drag-and-drop/reframe machinery (editor-only, meaningless in a static Next.js build), is what was worth porting.

**Decision:** Rebuilt `apps/web` as a real Next.js implementation of the Home design, faithfully — same palette (warm `#fbf9f6` canvas, `#171412` ink, `oklch(0.55 0.22 255)` accent blue), same type pairing (General Sans display, restored from git history at `dad1e2a~1`, + Geist body/mono), same section set and copy (hero, service-strip ticker, Who We Are, Ecosystem, Capabilities, Featured Work, Process, Testimonials, Insights, FAQ, Contact CTA), same scroll-reveal behavior (ported the design's own vanilla `IntersectionObserver` script into a `Reveal` client component instead of reintroducing `framer-motion`). Structured as composed Server Component sections under `src/components/home/`, with shared primitives: `ImageSlot` (real photo via `resolvePhoto()` when present, otherwise the honest dashed-frame empty state described above — replacing `GraphicPanel` entirely), `UndaMark`/`UndaSymbolDefs` (the logo, defined once via SVG `<symbol>`/`<use>`), and `Reveal`. `globals.css` ports the design's hand-written component classes (`.btn-primary`, `.card`, `.eco-card`, `.cap-row`, `.faq-item`, etc.) into a Tailwind v4 `@layer components` block specifically so utility classes on the same element reliably win the cascade (verified this was necessary: without the layer, `.h2`'s own `font-size`/`margin` beat a same-element Tailwind override because the component class was declared later in source order than Tailwind's utilities). One deviation from the literal file: nav/footer were pulled out of the Home markup into `layout.tsx`, since they're site-wide chrome, not Home-specific content — behaviorally identical, structured for the additional pages (About, Careers, etc.) this project already has designs for. One scope gap left open: the design's own responsive/mobile treatment wasn't specified for the nav (no hamburger pattern in the source file), so nav links are hidden below `md` for now rather than inventing a mobile menu the design didn't call for.

**Consequences:** Verified with `tsc --noEmit`, `biome check`, a full `next build`, and Playwright screenshots at both desktop (1440px) and mobile (390px) — including scroll-triggered reveal states, not just first paint, after the same "full-page screenshot outruns IntersectionObserver" artifact from ADR-024's verification showed up again here and needed the same scroll-through methodology to rule out. Every image slot (`who-we-are.jpg`, 3 work-card photos, 3 insight-card photos) is a placeholder pending real files at the exact paths documented in `apps/web/public/photography/README.md` — this time with a distinct caption per slot instead of one repeated logo mark, directly addressing the ADR-025 failure mode. `clsx` and `tailwind-merge` are back in `package.json` (for `cn()`); `framer-motion` and `lucide-react` were not reintroduced. About, Careers, Contact, Ecosystem, Insights, and Work all have corresponding approved designs in the same Claude Design project, not yet implemented — natural next scope once Home is confirmed.
