...
5. **Never fabricate a division accent color outside the four defined hues** (Royal Blue / Orange / Cyan / Emerald — see [Cross-brand rules](./design-system/rules/cross-brand-rules.md)), and never let a division override anything but its accent token. Typography, spacing, radius, motion, and component structure are shared and non-negotiable across Solutions, Studio, Technologies, and Travel.
